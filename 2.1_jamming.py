...
class MyEnv(gym.Env):
    ...
    description:
    0122
    拓宽频域,只改了step开头的部分。step末尾归一化和reset未处理；
    增加了可视化，修正了可视化颜色；
    载入正确的agent模型(0115)；
    问题：依旧不动。收敛反了吧。PPO-3
    """
    ...
    def step(self, action):
        ...
        f1 = rate.sum() / self.ue_num  # todo 最小化
        # 训练
        self.current_step += 1
        self.reward = f1  # todo 做差分 最大化差分；干扰改变前和干扰改变后；或者记录上一个reward；
...
if __name__ == "__main__":
    ...
    if train_mode:
        ...
        model.save("PPO_j_0122")  # todo
    else:
        env = MyEnv()
        t_num = env.t_num
        model = PPO.load("./PPO_j_0122", env=env)  # PPO_u_0103
        dispNum = 100
