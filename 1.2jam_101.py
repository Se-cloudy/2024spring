...
class MyEnv(gym.Env):
    ...
    def step(self, action):
        # 输入的是action = [action_u, action_j]
        # model_u.predict(observations=obs)  # TODO
        action_j = action[3:]  # jamming
        self.state = action  # [state_u, state_j]
...
if __name__ == "__main__":
    ...
    if train_mode:
        ...
        model_j.save("./model_j")
    else:
        model_u = PPO.load("model_u", env=env)
        model_j = PPO.load("model_j", env=env)
        obs = env.reset()
        num = int(1e2)  # 演示
        s1 = np.zeros((num,))
        for ii in range(num):
