...
class MyEnv(gym.Env):
    ...
    3 reset改了一下
    4 reward改了一下
    我还把所有的状态量和那个行为进行规划处理了，所有的状态量控制在零到一，所有的行为控制在负一到一，然后每进到step的时候，会把那个action和
    state转换一下，然后归，就是存储到那个八分儿的时候，会再归一化一下，这样的训练效果会比较好。
    """
    ...
    def step(self, action):
        ...
        self.state[3] = self.mu_j[self.tt]
        self.state[4] = self.sigma_j
        self.state[5] = self.p_j[self.tt]
        ...
        f1 = rate.sum() / self.ue_num  # 最大化 不用取负
    ...
    def reset(self):
        ...
        self.state[3] = (self.mu_j[self.tt] - (self.f0 - 0.5 * self.B)) / self.B
        self.state[4] = (self.sigma_j - 1) / (0.5 * self.B - 1)
        self.state[5] = self.p_j[self.tt] / self.p_jm
...
if __name__ == "__main__":
    ...
    if train_mode:
        ...
        # Train the agent
        model.learn(total_timesteps=int(5e6), callback=eval_callback)  # 5e6
        model.save("PPO_u_0102")
    else:
        env = MyEnv()
        model = PPO.load("./PPO_u_0102", env=env)
        dispNum = 1000
