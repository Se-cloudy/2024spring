...
from UAVMEC.top_layer_new.UAV_Env_V2 import UAV_env_V2
from UAVMEC.top_layer_new.UAV_Env_Multi import UAV_env_Multi
...
class TOP_ENV_DQN(gym.Env):
    ...
    def step(self, action):
        ...
        obs[0] = 0.
        obs[1] = 0.
        obs[2] = (self.state[2 + int(action)*2] - self.state[0])*30/40
        obs[3] = (self.state[2 + int(action)*2 + 1] - self.state[1])*30/40
        obs[4] = 1.
        obs[5] = E/E_max
        obs[6] = (T_max - T) / T_max
        obs = np.array(obs)
        for time in range(N+1):
            ...
            if dones:
                ...
                state_next[0] = obs[0]*40/30 + self.state[0]
                state_next[1] = obs[1]*40/30 + self.state[1]
                if info == False:
