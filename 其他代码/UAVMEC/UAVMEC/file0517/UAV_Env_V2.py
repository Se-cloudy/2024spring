...
class UAV_env_V2(gym.Env):
    ...
    def set_position(self):
        if self.flag == 0:
            self.state[UAV_num * 2 + 0 * 2] = 10./Radius
            self.state[UAV_num * 2 + 0 * 2 + 1] = 10./Radius
            self.state[UAV_num * 2 + 1 * 2] = 20./Radius
            self.state[UAV_num * 2 + 1 * 2 + 1] = -20./Radius
            self.flag = 1
            return 0

        if self.flag == 1:
            self.state[UAV_num * 2 + 0 * 2] = -5./Radius
            self.state[UAV_num * 2 + 0 * 2 + 1] = 10./Radius
            self.state[UAV_num * 2 + 1 * 2] = -20./Radius
            self.state[UAV_num * 2 + 1 * 2 + 1] = -20./Radius
            self.flag = 2
        ...
        if self.flag == 2:
            self.state[UAV_num * 2 + 0 * 2] = 0./Radius
            self.state[UAV_num * 2 + 0 * 2 + 1] = 5./Radius
            self.state[UAV_num * 2 + 1 * 2] = 5./Radius
            self.state[UAV_num * 2 + 1 * 2 + 1] = -20./Radius
            self.flag = 0
            return 0
    ...
    def reset(self):
        ...
        # theta1 = np.random.uniform(0, 0.5 * np.pi)
        # theta2 = np.random.uniform(0.5 * np.pi, 1 * np.pi)
        # Distance = np.random.uniform(0, Radius)
        # self.state[2] = np.cos(theta1) * Distance / Radius
        # self.state[3] = np.sin(theta1) * Distance / Radius
        # self.state[4] = np.cos(theta2) * Distance / Radius
        # self.state[5] = np.sin(theta2) * Distance / Radius
        ...
        self.state[2*(UAV_num+User_num) + User_num] = 1.
    ...
    def step(self, action):
        ...
        reward = 0
        for j in range(User_num):
            if(self.state[(UAV_num + User_num) * 2 + j] != 0):
                g = g0 / (np.power((state_next[0] - state_next[UAV_num * 2 + j * 2])*Radius, 2) + np.power((state_next[1] - state_next[UAV_num * 2 + j * 2 + 1])*Radius, 2) + np.power(H, 2))
                reward += 10*(action[2+j]/Lc_max)*(g/g_max - 0.95)
