...
class UAV_env_Multi(gym.Env):
    ...
    def reset(self):
        ...
        for j in range(User_num):
            ...
            self.state[UAV_num * 2 + j * 2] = 25/Radius
        ...
        self.state[2 * (UAV_num + User_num) + User_num + 1] = 0.6/T
    ...
    def step(self, action):
        ...
        reward = 2*(action[2]/Lc_max)*(g/g_max-0.9) + (-1/9.21*np.log(alpha*(1-self.state[2*(UAV_num+User_num)+User_num+1])+0.0001))*(action[2]/Lc_max)  #能量时间对数障碍
