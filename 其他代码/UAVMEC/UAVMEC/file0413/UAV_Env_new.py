...
class UAV_env_new(gym.Env):
    ...
    def __init__(self, grid_size=10):
        super(UAV_env_new, self).__init__()
    ...
    def step(self, action):
        ...
        #reward = -P_tr*delta*10 - self.state[(UAV_num + User_num) * 2 + j]/50
        reward = sum(sum(rs_next))/rs_max
        #reward = -self.state[(UAV_num + User_num) * 2 + j]*L_max/sum(sum(rs_next))*P_tr
        #reward = -L_max / sum(sum(rs_next)) * P_tr
        if sum(state_next[(UAV_num + User_num) * 2:(UAV_num + User_num) * 2 + User_num]) == 0:
            done = True
            reward += (N - self.current_step)*sum(sum(rs_next))/rs_max
            # reward += 1
        else:
            done = False
