...
class TOP_ENV(gym.Env):
    ...
    def step(self, action):
        ...
        for i in range(User_num):
            state_next[2 * (User_num + UAV_num) + i] = self.state[2 * (User_num + UAV_num) + i]

        for i in range(UAV_num):
            state_next[2 * (User_num + UAV_num) + int(action[i])] = state_next[2*(User_num+UAV_num) + int(action[i])] + 1
            if(state_next[2 * (User_num + UAV_num) + int(action[i])] > 1):
                # 更新用户位置（保持不变）
                for j in range(User_num):
                ...
                for k in range(UAV_num):
                    state_next[2*k] = self.state[2*k]
                    state_next[2*k + 1] = self.state[2*k + 1]

                self.state = state_next
                reward = -10*(1-(self.current_step-1)/(User_num/UAV_num))
                done = True
