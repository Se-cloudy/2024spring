...
class TOP_ENV(gym.Env):
    ...
    def __init__(self, grid_size=10):
        ...
        self.trajectory = np.zeros((UAV_num, User_num + 1, 2), dtype=np.float32)
    ...
    def reset(self):
        ...
        for i in range(UAV_num):
            self.state[2*i] = 0. / Area_Bound
            self.state[2*i+1] = 0. / Area_Bound
    ...
    def step(self, action):
        for i in range(UAV_num):
            self.trajectory[i][self.current_step][0] = self.state[0]*Area_Bound
            self.trajectory[i][self.current_step][1] = self.state[1]*Area_Bound
