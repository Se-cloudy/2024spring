...
STATE_DIM = 2*(UAV_num+User_num) + User_num + 2
ACTION_DIM = UAV_num
...
class TOP_ENV_DQN(gym.Env):
    ...
    def reset(self):
        ...
        self.energy_budget = 3000
        self.state[2*(User_num+UAV_num)+User_num] = self.energy_budget/E_max

        self.time = np.random.uniform(0.4, T_max)
        self.time = 2
        self.state[2*(User_num+UAV_num)+User_num+1] = self.time/T_max
    ...
    def step(self, action):
        ...
        obs[4] = 1.
        obs[5] = self.state[2*(User_num+UAV_num)+User_num]
        obs[6] = self.state[2*(User_num+UAV_num)+User_num+1]
        obs = np.array(obs)
        for time in range(N):
            ...
            if dones:
                ...
                Total_reward = 1-Total_Com_Energy/100
        ...
        #更新时间和能量
        state_next[2*(User_num+UAV_num)+User_num] = self.state[2*(User_num+UAV_num)+User_num]
        state_next[2*(User_num+UAV_num)+User_num+1] = self.state[2*(User_num+UAV_num)+User_num+1]
