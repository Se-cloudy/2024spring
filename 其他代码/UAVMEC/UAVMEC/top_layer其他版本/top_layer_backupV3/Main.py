...
from UAVMEC.top_layer.Top_Env_DQN_5 import TOP_ENV_DQN_five
from UAVMEC.top_layer.UAV_Env_V2 import UAV_env_V2
from UAVMEC.top_layer.Reset import Reset_Env
...
def planning(i, action, area_position, area_No, point_position, area_bound):
    ...
    if area_No[action] > 4:
        ...
        #判断是否带有切入点
        if point_position[0]!=0 and point_position[1]!=0:
            area_position_next[4, 0] = point_position[0]
            area_position_next[4, 1] = point_position[1]
            area_No_next[4] = 1

            # 初始化
            state = np.zeros(2*6+5, dtype=np.float32)
            ...
            state[2 + 2*4] = point_position[0]
            state[2 + 2*4 + 1] = point_position[1]
            ...
            _ = env_5.reset()
            while not dones:
                env_5.give_value(state)
                action, State_next = model_5.predict(state, deterministic=True)
                State_next, rewards, dones, info = env_5.step(action)
                series[ID] = action
                ID += 1
                state = State_next
        ...
        if point_position[0]!=0 and point_position[1]!=0:
            ...
            if area_No[action]<4:
                for j in range(4-area_No[action]):
                    state[2 + 2*5 + area_No[action] + j] = 1
            #开始规划
            _ = env_5.reset()
            while not dones:
                env_5.give_value(state)
                ...
                Trajectory_No[i] += 1
                for t in range(env_5.uav_tra_No):
                    Final_Trajectory[i, Final_Trajectory_No[i], 0] = env_5.uav_tra[t][0]
                    Final_Trajectory[i, Final_Trajectory_No[i], 1] = env_5.uav_tra[t][1]
                    Final_Trajectory_No[i] += 1
                Energy_Com[i] += env_5.Total_Com_Energy
                Energy_Fly[i] += env_5.Total_Fly_Energy
            #更新无人机位置
...
if __name__ == '__main__':
    ...
    model_5 = DQN.load("./Top_best/best_model_5_backup", env=env_5)
