...
class TOP_ENV_DQN_five(gym.Env):
    """
    ...
    def __init__(self, grid_size=10):
        super(TOP_ENV_DQN_five, self).__init__()
    ...
    def reset(self):
        ...
        for j in range(User_num):
            self.state[2*UAV_num + j*2] = np.random.uniform(-Area_Bound, Area_Bound)/Area_Bound/2
            self.state[2*UAV_num + j*2 + 1] = np.random.uniform(-Area_Bound, Area_Bound)/Area_Bound/2
