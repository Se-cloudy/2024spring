...
class UAV_env_Multi(gym.Env):
    ...
    def __init__(self, grid_size=10):
        ...
        self.observation_space = spaces.Box(low=np.array([0,0,0,0,0,0,0]), high=np.array([Radius,Radius,Radius,Radius,L_max,E_max,T]), dtype=np.float32)
        self.seed()
        self.current_step = 0
        self.rs_max = 0
    ...
    def reset(self):
        ...
        for i in range(UAV_num):
            self.state[i * 2] = 0./Radius
            self.state[i * 2 + 1] = 0./Radius

        for j in range(User_num):
            theta = np.random.uniform(0, 2*np.pi)
            #Distance = np.random.uniform(0, Radius)
            Distance = 10
            #self.state[UAV_num * 2 + j * 2] = 0.20714194
            #self.state[UAV_num * 2 + j * 2 + 1] = 0.20516011
        ...
        self.energy_budget = 700
        #self.energy_budget = np.random.uniform(100, E_max)
        self.state[2*(UAV_num+User_num) + User_num] = self.energy_budget/E_max

        #self.time = np.random.uniform(0.1, 2.5)
        #self.state[2 * (UAV_num + User_num) + User_num + 1] = self.time/T
        self.state[2 * (UAV_num + User_num) + User_num + 1] = 0.1/T
    ...
    def step(self, action):
        ...
        # 能量对速度施加约束
        alpha = 1.5*self.state[2*(UAV_num+User_num)+User_num] * (self.state[2*(UAV_num+User_num)+User_num+1])
        ...
        FlyingEnergy = 0.5*Mass*delataV*delataV*delta #+ 10*Mass*delta
        ...
        #reward = 2*(action[2]/Lc_max)*(g/g_max-0.9) + (-3/10000*np.log((1-self.state[2*(UAV_num+User_num)+User_num+1])+0.0001))*(action[2]/Lc_max)  #时间对数障碍

        if distance <= 5:
            reward = reward + (5-distance)/5
        ...
        if state_next[2 * (UAV_num + User_num) + User_num + 1] >= 1:
            ...
            reward -= 100*state_next[2*(UAV_num+User_num)]
            self.E_com += 1000*state_next[2*(UAV_num+User_num)]
        # if state_next[2 * (UAV_num + User_num) + User_num] <= 0:
        #     done = True
