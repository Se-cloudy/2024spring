...
class UAV_env(gym.Env):
    ...
    def reset(self):
        ...
        for j in range(User_num):
            ...
            # self.state[UAV_num * 2 + j * 2] = 0.01088455
            # self.state[UAV_num * 2 + j * 2 + 1] = 0.23928952
        ...
        #self.time = np.random.uniform(1.5, T)
        self.state[2 * (UAV_num + User_num) + User_num + 1] = 0
    ...
    def step(self, action):
        ...
        reward = 2*(action[2]/Lc_max)*(g/g_max - 0.9) + (-0.000003/(state_next[2 * (UAV_num + User_num) + User_num + 1]-1)-0.000003)*(action[2]/Lc_max)#*(0.000003/(g/g_max)+1)
        if distance <= 0.5:
