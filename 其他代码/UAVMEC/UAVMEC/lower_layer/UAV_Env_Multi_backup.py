...
class UAV_env_Multi(gym.Env):
    ...
    def reset(self):
        ...
        for j in range(User_num):
            ...
            self.state[UAV_num * 2 + j * 2] = -5/Radius
            self.state[UAV_num * 2 + j * 2 + 1] = 2/Radius
    ...
    def step(self, action):
        ...
        #reward = 2*(action[2]/Lc_max)*(g/g_max - 0.9) + (-0.000001/(self.state[2 * (UAV_num + User_num) + User_num + 1]-1)-0.000001)*(action[2]/Lc_max)
        reward = 2*(action[2]/Lc_max)*(g/g_max-0.9) + (-3/1000000*np.log(1-self.state[2*(UAV_num + User_num) + User_num + 1]))*(action[2]/Lc_max)
