...
class UAV_env_V2(gym.Env):
    ...
    def __init__(self, grid_size=10):
        ...
        self.Combine_Data_size = np.zeros((User_num, N*User_num, 2), np.float32)
    ...
    def reset(self):
        ...
        self.state_raw[2] = 45./Radius
        self.state_raw[3] = 30./Radius
        # self.state_raw[4] = 15./Radius
        # self.state_raw[5] = 8./Radius
        # self.state_raw[6] = 25. / Radius
        # self.state_raw[7] = -20. / Radius

        for j in range(User_num):
            self.state_raw[(UAV_num + User_num) * 2 + j] = 1.

        for i in range(N*User_num):
            self.combine_tra[i][0] = 0.
            self.combine_tra[i][1] = 0.

        for j in range(User_num):
            for i in range(N*User_num):
                self.Combine_Data_size[j][i][0] = 0.
                self.Combine_Data_size[j][i][1] = 0.
    ...
    def reset_single(self,obs, initial_position):
        ...
        self.UAV_initial_position[0] = initial_position[0]
        self.UAV_initial_position[1] = initial_position[1]
    ...
    def combine(self, user_ID):
        for i in range(self.current_step):
            ...
            for j in range(User_num):
                if j==user_ID:
                    self.Combine_Data_size[j][self.tra_num][1] = self.Data_size[i][1]
                self.Combine_Data_size[j][self.tra_num][0] = self.tra_num
            self.tra_num += 1
