...
class UAV_env_V3(gym.Env):
    ...
    def __init__(self, grid_size=10):
        super(UAV_env_V3, self).__init__()
    ...
    def step(self, obs, action):
        ...
        #计算传输能耗
        self.Com_Energy = self.calculate_energy(action[2],state_next)

        #计算飞行能耗
        vx = (state_next[1] - self.state[1])*Radius/delta
        vy = (state_next[0] - self.state[0])*Radius/delta
        v2 = np.power(vx ,2) + np.power(vy ,2)
        self.Uav_Energy = 0.5*v2*Mass*delta
